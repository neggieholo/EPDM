...
import { fetchFavProjects, fieldMappings, filterEmptyValues } from "@/app/utils/ProjectFetchers";
...
export default async function Page() {
    ...
    try {
        ...
        const favRes:Record<string, any> = await fetch(`${baseUrl}/api/projects/getUserfavourites`, {
            headers: sessionCookie ? { cookie: `express.sid=${sessionCookie}` } : undefined,
            cache: "no-store",
        });
        const favData: { favProjects: string[] } = await favRes.json();
        const favs = favData.favProjects || [];

        // Filter only favourite projects
        const favoriteProjects = projectData.projects?.filter(project =>
            project["Project ID"] && favs.includes(project["Project ID"].toString())
        );

        // Optional: replace projects with only favourites
        projectData.projects = favoriteProjects;
    ...
    return mobileCheck
        ? <MobProjectsPage projects={projectData.projects || []} subscription={projectData.subscription} returnPage="All Favorites" />
        : <ProjectsPage projects={projectData.projects || []} subscription={projectData.subscription} returnPage="All Favorites"/>;
}

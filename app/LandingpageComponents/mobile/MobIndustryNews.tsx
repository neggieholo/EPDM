...
const MobNewsLinks: React.FC<NewsLinksProps> = ({ links }) => {
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 6;

    if (!links || links.length === 0) {
        return (
            <section className="w-full min-h-screen bg-white flex flex-col items-center py-10 relative pt-20">
        ...
        <section className="w-full min-h-screen bg-white flex flex-col items-center py-10 relative pt-20">
            ...
            <div className="w-full flex items-center justify-center p-4 mb-8">
                <div className="flex flex-col justify-evenly shadow-strong bg-primary items-center text-center h-auto py-8 px-5">
                    <h2 className="text-3xl font-extrabold mb-2 text-white leading-tight">
                        INDUSTRY
                    </h2>
                    <h2 className="text-3xl font-extrabold mb-2 text-white leading-tight">
                        NEWS
            ...
            <div className="w-[95%] bg-accent/50 border border-accent/20 p-1">
                <div className="overflow-y-auto pr-2 space-y-4 h-[85vh] py-4">
                    {currentLinks.map((link) => (
                        <div
                            key={link._id}
                            className="flex justify-between items-center p-4 bg-white rounded-lg shadow-sm 
                            hover:shadow-md transition border border-accent/10"
                        >
                            ...
                            <a
                                ...
                                className="text-accent hover:scale-110 transition-transform text-md"
                                title="Open link"
                            >
                                🔗
                            </a>
